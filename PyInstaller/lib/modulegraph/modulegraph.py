...
class ModuleGraph(ObjectGraph):
    ...
    def import_hook(self, name, caller=None, fromlist=None, level=-1):
        ...
        package, tail = self.find_head_package(parent, name, level)
        m = self.load_tail(package, tail)
        modules = [m]
    ...
    def find_module(self, name, path, parent=None):
        ...
        file_handle, filename, file_metadata = find_module(name, path)

        try:
            if filename:
                filename = os.path.realpath(filename)

            return (file_handle, filename, file_metadata)
        except:
            file_handle.close()
            raise
