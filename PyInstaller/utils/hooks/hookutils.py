...
def get_pywin32_module_file_attribute(module_name):
    """
    Get the absolute path of the PyWin32-specific module with the passed name.

    This function is intended to by called by hook functions handling the
    PyWin32-specific `pywintypes` and `pythoncom` modules. On importation, both
    modules attempt to internally import corresponding shared libraries (e.g.,
    under Python 3.4, `pywintypes.py` imports `pywintypes34.dll`). Since most
    Python Windows distributions (including Anaconda) install these libraries to
    non-standard directories, this usually fails with exceptions resembling
    `ImportError: No system module 'pywintypes' (pywintypes34.dll)`. This is a
    well-known and currently unresolved issue with PyWin32.

    This function detects this failure, manually searches for these libraries,
    adds their parent directories to `sys.path`, and retries.
...
try:
    ...
    if str(exc).startswith('No system module'):
        # Force PyWin32 to search "sys.path" for DLLs. By default, PyWin32 only
        # searches "sys.prefix", Windows system directories (e.g.,
        # "C:\Windows\System32"), and the "site-packages\win32\lib" directory.
        # This is an ugly hack, but there literally exists no other way.
        sys.frozen = True
        ...
        for sys_path in sys_paths:
            # Absolute path of the directory possibly containing PyWin32 DLLs.
    ...
    # If this module remains unimportable, PyWin32 is probably not installed.
    if module is None:
        raise
