...
def get_matplotlib_backend_module_names():
    ...
    ----------
    list
        List of the fully-qualified names of all matplotlib backend modules.
    """
...
try:
    ...
    # this output from being erroneously interpreted as an error.
    ...
    # If this is an ImportError, print this exception's message without printing
    # a traceback. (ImportError messages are usually human-readable and thus
    # require no tracebacks for additional context.)
    if isinstance(exc, ImportError):
        print(str(exc))
    # Else, print this exception complete with a traceback. (Note that
    # traceback.print_exc() prints to stderr rather than the desired stdout.)
    else:
        import traceback
