...
def _collect_tcl_tk_files(mod):
    ...
    if not tcl_root:
        logger.error('Tcl/Tk not properly installed on this system.')
        return []
    ...
    # Tcl/Tk data files and subdirectories to be excluded.
    exclude_paths = ['demos', '*.lib', 'tclConfig.sh']

    tcltree = Tree(tcl_root, prefix='tcl', excludes=exclude_paths)
    tktree = Tree(tk_root, prefix='tk', excludes=exclude_paths)
    return (tcltree + tktree)


# Get the Tcl/Tk data files for bundling with the executable.
def hook(mod):
    # If not a supported platform, skip TCL/TK detection.
